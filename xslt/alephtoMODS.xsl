...
<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" 
    ...
    <!-- *These are the namespaces needed by FSU Special Collections* -->
    ...
    <!-- *Modifications made by FSU Libraries* -->
    <!-- Removed http://www.loc.gov/mods/v3 from xsi:schemaLocation because the result did not validate. mag 2014/05/28 -->
    <!-- Added or @code='5' to <xsl:for-each select="marc:subfield[not(@code='6' or @code='8' or @code='5')]"> in the <note> 
        template (approx. line 3059) so $5 field would not be in note fields. mag 2014/05/28 -->
    <!-- Added normalize-space for place elements (MARC 300 $a). mag 2014/05/28 -->
    <!-- Commented out template for geographic Code (approx. lines 405 and 3131_ . mag 2014/06/05 -->
    <!-- Copied mods:stripPunctuation and modified it to mods:stripPunctPlace so that place name abbreviations retain the ending period   -->
    <!-- Added or @tag='264' to <xsl.if>(approx. line 1359). mag 2014/06/05-->
    <!-- Changed stripPunctuation function on 260$a for PlaceTerm so that it doesn't strip out the ending period of place abbreviations. 
        Modified code: <xsl:value-of select="mods:stripPunctuation(.,',:; ')"/> Was: <xsl:value-of select="mods:stripPunctuation(.). mag 2014/06/05 -->
		  
	 <!-- *Modifications by Matthew Miguez FSU University Libraries* -->
	 <!-- Changed xmlns:local="http://www.loc.gov/mods/v3/local" to xmlns:mods="http://www.loc.gov/mods/v3"  mrm 2015-01-30  -->
     <!-- LCC# maps to IID 2015-02-02: default choice is 050, in its abscence, 090 -->
     <!-- FLVC namespaces $ extension added 2015-02-02 -->
    ...
    <!-- 
		...
		Revision 2.0 - Upgraded XSLT 1.0 1.78 stylesheet to XSLT 2.0 - 2012/06/25 ws
			...
			 			245 $c added element <note type="statement of responsibility"> 
						...
						mods:stripPunctation function to tests for initials and leaves punctuation if needed.						
    ...
    <xsl:function name="mods:stripPunctuation">
        <xsl:param name="str"/>
            ...
            <xsl:when test="contains($punctuation,$strEnd)">
                <xsl:choose>
                    ...
                    <xsl:when
                        ...
                        <xsl:value-of
                            select="normalize-space(substring($str,1,string-length($str)-3))"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of
                            select="normalize-space(substring($str,1,string-length($str)-1))"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
    ...
    <!-- 
		Strips punctuation from the last character of a field, this version accepts two parameters:
		 ...
		 $pcodes: optional punctuation to override the default, if empty uses defaults
	 -->
    <xsl:function name="mods:stripPunctuation">
        <xsl:param name="str"/>
	...
	-->
    <xsl:function name="mods:subfieldSelect">
	...
	-->
    <xsl:function name="mods:specialSubfieldSelect">
        <xsl:param name="datafield" as="node()"/>
        ...
        <xsl:variable name="subfieldStr">
            <xsl:for-each
                select="$datafield/child::*[contains($anyCodes, @code)      or (contains($beforeCodes,@code) and following-sibling::*[@code=$axis])       or (contains($afterCodes,@code) and preceding-sibling::*[@code=$axis])]">
                <xsl:apply-templates/>
                <xsl:text> </xsl:text>
            </xsl:for-each>
        </xsl:variable>
    ...
    <xsl:template match="marc:record">
        ...
        <mods xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-4.xsd">
            ...
            <xsl:apply-templates
                ...
                mode="abstract"/>
            <!-- Call table of contents template-->
            <xsl:apply-templates
		...
		-->
            <xsl:apply-templates
                select="marc:datafield[@tag='245'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'245')]     | marc:datafield[@tag='362'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'362')]     | marc:datafield[starts-with(@tag,'5')] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'5')]"
                mode="note"/>
            ...
            <xsl:apply-templates
                select="marc:datafield[@tag='760'] | marc:datafield[@tag='765'] |      marc:datafield[@tag='767'] | marc:datafield[@tag='762'] |      marc:datafield[@tag='770'] | marc:datafield[@tag='774'] | marc:datafield[@tag='775'] |     marc:datafield[@tag='772'] | marc:datafield[@tag='773'] |     marc:datafield[@tag='776'] | marc:datafield[@tag='780'] |     marc:datafield[@tag='785'] | marc:datafield[@tag='786']"
                mode="relatedItem"/>
...
<!--            <xsl:apply-templates
            ...
            <!-- Call for FSU IID construction  -->
            <xsl:apply-templates select="marc:datafield[@tag='050'] | marc:datafield[@tag='090']" />
				<!-- <xsl:apply-templates select="marc:datafield[@tag='090']" mode="id"/> 
            <xsl:apply-templates
    ...
    <xsl:template
        match="marc:datafield[@tag='130'] | marc:datafield[@tag='880'][@ind2!='2'][starts-with(marc:subfield[@code='6'],'130')]   | marc:datafield[@tag='730'][@ind2!='2'] | marc:datafield[@tag='880'][@ind2!='2'][starts-with(marc:subfield[@code='6'],'730')]"
        mode="titleInfo">
        <titleInfo type="uniform">
            <!-- Template checks for altRepGroup - 880 $6 -->
            <xsl:call-template name="xxx880"/>
            <title>
                <xsl:value-of select="mods:stripPunctuation(mods:subfieldSelect(.,'adfklmors'))"/>
            </title>
    ...
    <xsl:template
        ...
        <titleInfo type="abbreviated">
            ...
            <xsl:call-template name="xxx880"/>
            <title>
                <xsl:value-of select="mods:stripPunctuation(mods:subfieldSelect(.,'a'))"/>
            </title>
            <!-- subtitle -->
    ...
    <xsl:template match="marc:datafield[@tag='242']" mode="titleInfo">
        <titleInfo type="translated">
            ...
            <xsl:variable name="title">
                <xsl:value-of select="mods:subfieldSelect(.,'a')"/>
            </xsl:variable>
            ...
            <xsl:choose>
                ...
                <xsl:when test="@ind2 != '0'">
                    <nonSort>
                        <xsl:value-of
                            select="substring(mods:stripPunctuation($title),1,(@ind2 cast as xs:integer))"
                        />
                    </nonSort>
                    <title>
                        <xsl:value-of
                            select="substring(mods:stripPunctuation($title),(@ind2 cast as xs:integer)+1)"
                        />
                    </title>
                </xsl:when>
                <xsl:otherwise>
                    <title>
                        <xsl:value-of select="mods:stripPunctuation($title)"/>
                    </title>
                </xsl:otherwise>
            </xsl:choose>
            <!-- Subtitle -->
            ...
            <xsl:variable name="title">
                <xsl:choose>
                    <xsl:when test="marc:subfield[@code='b']">
                        ...
                        <xsl:value-of select="mods:specialSubfieldSelect(.,'','b','afgks','')"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="mods:subfieldSelect(.,'afgks')"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <!-- Estabilish nonSort element when second indicator is not equal to 0 -->
        ...
        <titleInfo>
            ...
            <xsl:if test="marc:subfield[@code='i']">
                <xsl:attribute name="displayLabel">
                    <xsl:value-of select="mods:subfieldSelect(.,'i')"/>
                </xsl:attribute>
            </xsl:if>
            <!-- Template checks for altRepGroup - 880 $6 -->
            <xsl:call-template name="xxx880"/>
            <!-- Create title -->
            <title>
                <xsl:value-of select="mods:stripPunctuation(mods:subfieldSelect(.,'af'))"/>
            </title>
            <!-- Subtitle -->
            <xsl:apply-templates select="marc:subfield[@code='b']" mode="titleInfo"/>
            <!-- Part number -->
    ...
    <xsl:template
        ...
        <titleInfo type="uniform">
            ...
            <!-- Create title -->
            <title>
                <xsl:value-of select="mods:stripPunctuation($str)"/>
                <!--				<xsl:value-of select="mods:stripPunctuation(mods:subfieldSelect(.,'adfklmors'))"/>-->
            </title>
            <!-- Subtitle -->
            <xsl:apply-templates select="marc:subfield[@code='b']" mode="titleInfo"/>
    ...
    <xsl:template
        ...
        <titleInfo type="alternative">
            ...
            <!-- Title -->
            <title>
                <xsl:value-of select="mods:stripPunctuation(mods:subfieldSelect(.,'ah'))"/>
            </title>
            <!-- Subtitle -->
            <xsl:apply-templates select="marc:subfield[@code='b']" mode="titleInfo"/>
            <!-- Part number -->
    ...
    <xsl:template match="marc:subfield[@code='b']" mode="titleInfo">
        <subTitle>
            ...
            <xsl:value-of
                select="mods:stripPunctuation(mods:specialSubfieldSelect(parent::*,'b','b','','afgk'))"
            />
        </subTitle>
    </xsl:template>
    ...
    <xsl:template match="marc:subfield[@code='n']" mode="titleInfo">
        <xsl:variable name="partNumber">
            <xsl:choose>
                <xsl:when
                    ...
                    <xsl:value-of
                        select="mods:specialSubfieldSelect(parent::*,'n','n','','fgkdlmor')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates/>
                </xsl:otherwise>
        ...
        <xsl:if test="string-length($partNumber) > 0">
            <partNumber>
                <xsl:value-of select="mods:stripPunctuation($partNumber)"/>
            </partNumber>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="marc:subfield[@code='p']" mode="titleInfo">
        <xsl:variable name="partName">
            <xsl:choose>
                <xsl:when
                    test="parent::*[@tag='245'] or parent::*[@tag='240'] or parent::*[@tag='130'] or parent::*[@tag='730']">
                    <xsl:value-of
                        select="mods:specialSubfieldSelect(parent::*,'p','p','','fgkdlmor')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates/>
                </xsl:otherwise>
        ...
        <xsl:if test="string-length($partName) > 0">
            <partName>
                <xsl:value-of select="mods:stripPunctuation($partName)"/>
            </partName>
        </xsl:if>
    </xsl:template>
    <!-- name templates -->
    ...
    <xsl:template
        match="marc:datafield[@tag='100'][not(marc:subfield[@code='t'])] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'100')][not(marc:subfield[@code='t'])]   | marc:datafield[@tag='700'][not(marc:subfield[@code='t'])] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'700')][not(marc:subfield[@code='t'])]"
        mode="name">
        <name>
            <!-- name attributes -->
            <xsl:choose>
            ...
            <xsl:for-each select="marc:subfield[@code='a']">
                <namePart>
                    <xsl:value-of
                        select="mods:stripPunctuation(mods:subfieldSelect(parent::*,'aq'))"/>
                </namePart>
            </xsl:for-each>
    ...
    <!-- 110 710 name-->
    <xsl:template
        match="marc:datafield[@tag='110'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'110')]   | marc:datafield[@tag='710'][not(marc:subfield[@code='t'])] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'710')][not(marc:subfield[@code='t'])]"
        mode="name">
        <xsl:if test="marc:subfield[@code!='t']">
            <name type="corporate">
                ...
                <xsl:if
                    ...
                    <namePart>
                        <xsl:value-of select="mods:subfieldSelect(.,'cdn')"/>
                    </namePart>
                </xsl:if>
                <!-- role -->
                <xsl:apply-templates select="marc:subfield[@code='e'] | marc:subfield[@code='4']"
    ...
    <!-- 111 711 name -->
    <xsl:template
        match="marc:datafield[@tag='111'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'111')]   | marc:datafield[@tag='711'][not(marc:subfield[@code='t'])] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'711')][not(marc:subfield[@code='t'])]"
        mode="name">
        <xsl:if test="marc:subfield[@code!='t']">
            <name type="conference">
                ...
                <!-- namePart -->
                <namePart>
                    <xsl:value-of select="mods:subfieldSelect(.,'acdenq')"/>
                </namePart>
                <!-- role -->
                <xsl:apply-templates select="marc:subfield[@code='e'] | marc:subfield[@code='4']"
                    mode="role"/>
    ...
    <xsl:template
        ...
        <genre authority="marcgt">
            ...
            <!-- Template checks for altRepGroup - 880 $6 -->
            <xsl:call-template name="xxx880"/>
            <xsl:value-of select="mods:subfieldSelect(.,'abcdef','-')"/>
        </genre>
    </xsl:template>
    <!-- 2.12 ws: added new genre template for 336 field -->
    <xsl:template
    ...
    <xsl:template
        ...
        <genre authority="marcgt">
            ...
            <!-- Template checks for altRepGroup - 880 $6 -->
            <xsl:call-template name="xxx880"/>
            <xsl:value-of select="mods:subfieldSelect(.,'abvxyz','-')"/>
        </genre>
    </xsl:template>
    <!-- OrginInfo -->
    <xsl:template name="orginInfo">
        ...
        <xsl:if
            ...
            <originInfo>
                ...
                <xsl:variable name="dataField260c" select="mods:stripPunctuation($datafield260Str)"/>
        ...
        <xsl:if
            test="marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'260')]     | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'250')]/marc:subfield[@code='a']    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'044')]    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'046')]    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'033')]    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'310')]    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'321')]">
            <originInfo>
                <!-- Calls scriptCode template -->
                <xsl:for-each
                    select="marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'260')]       | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'250')]      | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'044')]      | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'046')]      | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'033')]      | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'310')]      | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'321')]">
                    <xsl:choose>
        ...
        <xsl:if test="@ind2 ='0' or @ind2 ='1'or @ind2 ='2' or @ind2 ='3'">
            <originInfo>
                ...
                <xsl:if test="marc:subfield[@code='a']">
                    <place>
                        <placeTerm type="text">
                            <xsl:value-of select="normalize-space(mods:subfieldSelect(.,'a'))"/>
                        </placeTerm>
                    </place>
                </xsl:if>
                <xsl:if test="marc:subfield[@code='b']">
                    <publisher>
                        <xsl:value-of select="normalize-space(mods:subfieldSelect(.,'b'))"/>
                    </publisher>
                </xsl:if>
                    ...
                    <xsl:choose>
                        <xsl:when test="@ind2='0'">
                            <dateOther type="production">
                                <xsl:value-of select="normalize-space(mods:subfieldSelect(.,'c'))"
                                />
                            </dateOther>
                        </xsl:when>
                        <xsl:when test="@ind2='1'">
                    ...
                    <placeTerm>
                        ...
                        <xsl:choose>
                            <xsl:when test="starts-with($str,'[')">
                                <xsl:value-of
                                    select="mods:stripPunctuation(mods:stripPunctuation(substring(.,2)),']')"
                                />
                            </xsl:when>
                            <!-- FSU modification: was <xsl:value-of select="mods:stripPunctuation(.)"/> -->
                            <xsl:otherwise>
                                <xsl:value-of select="mods:stripPunctuation(.,',:; ')"/>
                            </xsl:otherwise>
                        </xsl:choose>
    ...
    <xsl:template
        ...
        <frequency authority="marcfrequency">
            <xsl:value-of select="mods:subfieldSelect(.,'ab')"/>
        </frequency>
    </xsl:template>
    ...
    <xsl:template name="physicalDescription">
        ...
        <xsl:if
            test="marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'130')][child::*[@code='h']] or     marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'240')][child::*[@code='h']] or     marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'242')][child::*[@code='h']] or     marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'245')][child::*[@code='h']] or     marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'246')][child::*[@code='h']] or     marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'730')][child::*[@code='h']] or     marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'256')][child::*[@code='a']] or     marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'300')] or     marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'856')][child::*[code='q']]">
            <physicalDescription>
                <!-- Calls scriptCode template -->
                <xsl:for-each
                    select="marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'130')][child::*[@code='h']] |       marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'240')][child::*[@code='h']] |       marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'242')][child::*[@code='h']] |       marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'245')][child::*[@code='h']] |       marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'246')][child::*[@code='h']] |       marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'730')][child::*[@code='h']] |       marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'256')][child::*[@code='a']] |       marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'300')] |       marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'856')][child::*[code='q']]">
                    <xsl:choose>
                        <xsl:when test="position()[1]">
                            <xsl:call-template name="scriptCode"/>
                        </xsl:when>
                ...
                <xsl:apply-templates
                    select="marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'130')]/marc:subfield[@code='h']       | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'240')]/marc:subfield[@code='h']       | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'242')]/marc:subfield[@code='h']       | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'245')]/marc:subfield[@code='h']       | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'246')]/marc:subfield[@code='h']       | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'730')]/marc:subfield[@code='h']      | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'256')]/marc:subfield[@code='a']"
                    mode="physDesc"/>
                <xsl:apply-templates
    ...
    <xsl:template
        ...
        mode="physDesc">
        <extent>
            <xsl:value-of select="mods:subfieldSelect(.,'abce3fg')"/>
        </extent>
    </xsl:template>
    <!-- 351 note-->
    <xsl:template match="marc:datafield[@tag='351']" mode="physDesc">
        <note type="arrangement">
            ...
            <xsl:value-of select="mods:subfieldSelect(.,'abc')"/>
        </note>
    </xsl:template>
    ...
    <xsl:template name="form">
        ...
        <xsl:apply-templates
            select="marc:datafield[@tag='130']/marc:subfield[@code='h'] | marc:datafield[@tag='240']/marc:subfield[@code='h'] |     marc:datafield[@tag='242']/marc:subfield[@code='h'] | marc:datafield[@tag='245']/marc:subfield[@code='h']     | marc:datafield[@tag='246']/marc:subfield[@code='h'] | marc:datafield[@tag='730']/marc:subfield[@code='h']    | marc:datafield[@tag='256']/marc:subfield[@code='a'] | marc:datafield[@tag='337']/marc:subfield[@code='a'] | marc:datafield[@tag='338']/marc:subfield[@code='a']"
            mode="physDesc"/>
    </xsl:template>
	...
	-->
    <xsl:template
        match="marc:datafield[@tag='130']/marc:subfield[@code='h']    | marc:datafield[@tag='240']/marc:subfield[@code='h'] | marc:datafield[@tag='242']/marc:subfield[@code='h']    | marc:datafield[@tag='245']/marc:subfield[@code='h'] | marc:datafield[@tag='246']/marc:subfield[@code='h']    | marc:datafield[@tag='730']/marc:subfield[@code='h'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'130')]/marc:subfield[@code='h']    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'240')]/marc:subfield[@code='h']    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'242')]/marc:subfield[@code='h']    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'245')]/marc:subfield[@code='h']    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'246')]/marc:subfield[@code='h']    | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'730')]/marc:subfield[@code='h']"
        mode="physDesc">
        <form authority="gmd">
            <xsl:variable name="str" select="mods:stripPunctuation(.)"/>
    ...
    <xsl:template
        ...
        <tableOfContents>
            ...
            <xsl:call-template name="uri"/>
            <!-- Subfields  -->
            <xsl:value-of select="mods:subfieldSelect(.,'agrt')"/>
        </tableOfContents>
    </xsl:template>
    ...
    <xsl:template
        ...
        <accessCondition type="restrictionOnAccess">
            ...
            <xsl:call-template name="xxx880"/>
            <!-- Values of subfields -->
            <xsl:value-of select="mods:subfieldSelect(.,'abcd35')"/>
        </accessCondition>
    </xsl:template>
    <xsl:template
        ...
        <accessCondition type="useAndReproduction">
            ...
            <xsl:call-template name="xxx880"/>
            <!-- Values of subfields -->
            <xsl:value-of select="mods:subfieldSelect(.,'abcde35')"/>
        </accessCondition>
    </xsl:template>
    ...
    <xsl:template
        ...
        <note type="date/sequential designation">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'az')"/>
        </note>
    </xsl:template>
    <xsl:template
        ...
        <note type="thesis">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'abcdgo')"/>
        </note>
    </xsl:template>
    <xsl:template
    ...
    <xsl:template
        ...
        <note type="preferred citation">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'3a')"/>
        </note>
    </xsl:template>
    <xsl:template
        ...
        <note type="additional physical form">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'abcd')"/>
        </note>
    </xsl:template>
    <xsl:template
        ...
        <note type="reproduction">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'abcdefmn')"/>
        </note>
    </xsl:template>
    <!-- tmee 2.16
	<xsl:template match="marc:datafield[@tag='534'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'534')]" mode="note">
		<note type="original version">
			...
			<xsl:call-template name="xxx880"/>
			<xsl:call-template name="uri"/>
			<xsl:value-of select="mods:subfieldSelect(.,'pabcefklmnt')"/>
		</note>
	</xsl:template>
	-->
    <xsl:template
    ...
    <xsl:template
        ...
        <note type="system details">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'ai')"/>
        </note>
    </xsl:template>
    <xsl:template
        ...
        <note type="acquisition">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'3noabcdefh')"/>
        </note>
    </xsl:template>
    <xsl:template
    ...
    <xsl:template
        ...
        <note type="language">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'3ab')"/>
        </note>
    </xsl:template>
    <xsl:template
    ...
    <xsl:template
        ...
        <note type="version identification">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'3abcde')"/>
        </note>
    </xsl:template>
    <xsl:template
    ...
    <xsl:template
        ...
        <note type="action">
            ...
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:value-of select="mods:subfieldSelect(.,'abcdefhijklnoxz')"/>
        </note>
    </xsl:template>
    <xsl:template
    ...
    <!-- General Note field -->
    <xsl:template
        match="marc:datafield[@tag='500'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'500')]   | marc:datafield[@tag='501'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'501')]   | marc:datafield[@tag='507'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'507')]   | marc:datafield[@tag='513'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'513')]   | marc:datafield[@tag='514'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'514')]   | marc:datafield[@tag='516'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'516')]   | marc:datafield[@tag='522'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'522')]   | marc:datafield[@tag='525'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'525')]   | marc:datafield[@tag='526'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'526')]   | marc:datafield[@tag='542'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'542')]   | marc:datafield[@tag='544'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'544')]   | marc:datafield[@tag='547'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'547')]   | marc:datafield[@tag='550'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'550')]   | marc:datafield[@tag='552'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'552')]   | marc:datafield[@tag='555'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'555')]   | marc:datafield[@tag='556'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'556')]   | marc:datafield[@tag='563'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'563')]   | marc:datafield[@tag='565'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'565')]   | marc:datafield[@tag='567'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'567')]   | marc:datafield[@tag='580'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'580')]   | marc:datafield[@tag='584'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'584')]   | marc:datafield[@tag='586'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'586')]   | marc:datafield[@tag='588'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'588')]   | marc:datafield[starts-with(@tag,'59')]  | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'59')]"
        mode="note">
        <note>
            <!-- Template checks for altRepGroup - 880 $6 -->
            <xsl:call-template name="xxx880"/>
            <xsl:call-template name="uri"/>
            <xsl:for-each select="marc:subfield[not(@code='6' or @code='8' or @code='5')]">
                <xsl:apply-templates/>
    ...
    <xsl:template
        ...
        <xsl:if test="marc:subfield[@code='d' or @code='e' or @code='f' or @code='g']">
            <subject>
                ...
                <cartographics>
                    <coordinates>
                        <xsl:value-of select="mods:subfieldSelect(.,'defg')"/>
                    </coordinates>
                </cartographics>
            </subject>
        </xsl:if>
    ...
    <xsl:template
        match="marc:datafield[@tag='255'][child::*[@code='a' or @code='b' or @code='c']]   | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'255')][child::*[@code='a' or @code='b' or @code='c']]"
        mode="subject">
        <subject>
            <!-- Template checks for altRepGroup - 880 $6 -->
            <xsl:call-template name="xxx880"/>
        ...
        <subject>
            ...
            <name type="corporate">
                ...
                <xsl:if test="marc:subfield[@code='c' or @code='d' or @code='n' or @code='p']">
                    <namePart>
                        <xsl:value-of select="mods:subfieldSelect(.,'cdnp')"/>
                    </namePart>
                </xsl:if>
                <!-- role -->
                <xsl:apply-templates select="marc:subfield[@code='e'] | marc:subfield[@code='4']"
    ...
    <xsl:template
        ...
        <subject>
            ...
            <name type="conference">
                <namePart>
                    <xsl:value-of select="mods:subfieldSelect(.,'abcdeqnp')"/>
                </namePart>
                <!-- role -->
    ...
    <xsl:template
        ...
        <subject>
            ...
            <titleInfo>
                <title>
                    <xsl:value-of
                        select="normalize-space(mods:stripPunctuation(mods:subfieldSelect(.,'adfhklor')))"
                    />
                </title>
                <xsl:apply-templates select="marc:subfield[@code='n']" mode="titleInfo"/>
    ...
    <xsl:template
        ...
        <subject>
            ...
            <xsl:call-template name="uri"/>
            <temporal>
                <xsl:value-of select="mods:stripPunctuation(mods:subfieldSelect(.,'abcd'))"/>
            </temporal>
            <!-- Additional subjects -->
            <xsl:apply-templates
                select="marc:subfield[@code='v' or @code='x' or @code='y' or @code='z']"
    ...
    <xsl:template
        match="marc:datafield[@tag='662'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'662')]    | marc:datafield[@tag='752'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'752')]"
        mode="subject">
        <subject>
            <!-- Template checks for altRepGroup - 880 $6 -->
            <xsl:call-template name="xxx880"/>
    ...
    <xsl:template match="marc:subfield[@code='t']" mode="subjectTitle">
        <titleInfo>
            <title>
                <xsl:value-of select="normalize-space(mods:stripPunctuation(.))"/>
            </title>
    ...
    <xsl:template
        ...
        <classification authority="udc">
            <!-- Template checks for altRepGroup - 880 $6 -->
            <xsl:call-template name="xxx880"/>
            <xsl:value-of select="mods:subfieldSelect(.,'abx')"/>
        </classification>
    </xsl:template>
    <xsl:template
        ...
        <location>
            ...
            <xsl:if test="marc:subfield[@code='a' or @code='b' or @code='e' or  @code='u']">
                <physicalLocation>
                    ...
                    <xsl:value-of select="mods:subfieldSelect(.,'abje')"/>
                </physicalLocation>
            </xsl:if>
            <xsl:if
                test="marc:subfield[@code='h' or @code='i' or @code='j' or @code='k' or @code='l' or @code='m' or @code='t']">
                <shelfLocator>
                    <xsl:value-of select="mods:subfieldSelect(.,'hijklmt')"/>
                </shelfLocator>
            </xsl:if>
        </location>
    </xsl:template>
            ...
            <location>
                ...
                <url displayLabel="electronic resource">
                    ...
                    <xsl:if test="marc:subfield[@code='y' or @code='3']">
                        <xsl:attribute name="displayLabel">
                            <xsl:value-of select="mods:subfieldSelect(.,'y3')"/>
                        </xsl:attribute>
                    </xsl:if>
                    <xsl:if test="marc:subfield[@code='z']">
                        <xsl:attribute name="note">
                            <xsl:value-of select="mods:subfieldSelect(.,'z')"/>
                        </xsl:attribute>
                    </xsl:if>
        ...
        <relatedItem type="series">
            <xsl:for-each
                ...
                <titleInfo>
                    ...
                    <title>
                        <xsl:value-of
                            select="normalize-space(mods:stripPunctuation(mods:subfieldSelect(.,'a')))"
                        />
                    </title>
                    <xsl:if test="marc:subfield[@code='v']">
                        <partNumber>
                            <xsl:value-of select="mods:stripPunctuation(marc:subfield[@code='v'])"
                            />
                        </partNumber>
                    </xsl:if>
                </titleInfo>
        ...
        <relatedItem type="series">
            <xsl:for-each
                ...
                <titleInfo>
                    ...
                    <title>
                        <xsl:value-of
                            select="normalize-space(mods:stripPunctuation(mods:subfieldSelect(.,'av')))"
                        />
                    </title>
                </titleInfo>
            </xsl:for-each>
        ...
        <relatedItem type="isReferencedBy">
            <xsl:for-each
                ...
                <note>
                    <!-- Template checks for altRepGroup - 880 $6 -->
                    <xsl:call-template name="xxx880"/>
                    <xsl:value-of select="mods:stripPunctuation(mods:subfieldSelect(.,'c'))"/>
                </note>
            </xsl:for-each>
        </relatedItem>
    </xsl:template>
        ...
        <relatedItem>
            ...
            <xsl:for-each
                ...
                <titleInfo>
                    ...
                    <title>
                        <xsl:value-of
                            select="normalize-space(mods:stripPunctuation(mods:specialSubfieldSelect(.,'tfklmorsv','t','','g')))"
                        />
                    </title>
                ...
                <name type="personal">
                    ...
                    <namePart>
                        <xsl:value-of select="mods:specialSubfieldSelect(.,'aq','t','g','')"/>
                    </namePart>
            ...
            <xsl:for-each
                ...
                <name type="corporate">
                    ...
                    <xsl:variable name="tempNamePart">
                        <xsl:value-of select="mods:specialSubfieldSelect(.,'c','t','dgn','')"/>
                    </xsl:variable>
        ...
        <relatedItem>
            ...
            <xsl:for-each
                ...
                <titleInfo>
                    ...
                    <title>
                        <xsl:value-of
                            select="normalize-space(mods:stripPunctuation(mods:specialSubfieldSelect(.,'tfklsv','t','','g')))"
                        />
                    </title>
                    <xsl:apply-templates select="marc:subfield[@code='n']" mode="relatedItem"/>
                    <xsl:apply-templates select="marc:subfield[@code='p']" mode="relatedItem"/>
                ...
                <name type="conference">
                    <xsl:call-template name="xxx880"/>
                    <namePart>
                        <xsl:value-of select="mods:specialSubfieldSelect(.,'aqdc','t','gn','')"/>
                    </namePart>
                </name>
                <!-- physical description form -->
                <xsl:apply-templates select="marc:subfield[@code='h']" mode="relatedItem"/>
        ...
        <relatedItem>
            ...
            <xsl:for-each
                ...
                <titleInfo>
                    ...
                    <title>
                        <xsl:value-of
                            select="normalize-space(mods:stripPunctuation(mods:subfieldSelect(.,'adfgklmorsv')))"
                        />
                    </title>
                    <xsl:apply-templates select="marc:subfield[@code='n']" mode="relatedItem"/>
                    <xsl:apply-templates select="marc:subfield[@code='p']" mode="relatedItem"/>
    ...
    <xsl:template
        match="marc:datafield[@tag='760'] | marc:datafield[@tag='765'] |    marc:datafield[@tag='767'] | marc:datafield[@tag='762'] |    marc:datafield[@tag='770'] | marc:datafield[@tag='774'] | marc:datafield[@tag='775'] |   marc:datafield[@tag='772'] | marc:datafield[@tag='773'] |   marc:datafield[@tag='776'] | marc:datafield[@tag='780'] |   marc:datafield[@tag='785'] | marc:datafield[@tag='786']"
        mode="relatedItem">
        <xsl:variable name="s6" select="substring(normalize-space(marc:subfield[@code='6']), 5, 2)"/>
        <relatedItem>
        ...
        <relatedItem type="series">
            <xsl:for-each
                ...
                <name type="personal">
                    ...
                    <namePart>
                        <xsl:value-of
                            select="mods:stripPunctuation(mods:specialSubfieldSelect(.,'aq','t','g',''))"
                        />
                    </namePart>
                    <xsl:apply-templates select="marc:subfield[@code='b']" mode="termsOfAddress"/>
                    <xsl:apply-templates select="marc:subfield[@code='d']" mode="nameDate"/>
        ...
        <relatedItem type="series">
            <xsl:for-each
                ...
                <titleInfo>
                    ...
                    <title>
                        <xsl:value-of
                            select="normalize-space(mods:stripPunctuation(mods:specialSubfieldSelect(.,'tfklmorsv','t','','dg')))"
                        />
                    </title>
                    <xsl:apply-templates select="marc:subfield[@code='n']" mode="relatedItem"/>
                    <xsl:apply-templates select="marc:subfield[@code='p']" mode="relatedItem"/>
                ...
                <name type="corporate">
                    ...
                    <namePart>
                        <xsl:value-of
                            select="mods:stripPunctuation(mods:specialSubfieldSelect(.,'c','t','dgn',''))"
                        />
                    </namePart>
                    <xsl:apply-templates
                        select="marc:subfield[@code='e'] | marc:subfield[@code='4']" mode="role"/>
        ...
        <relatedItem type="series">
            <xsl:for-each
                ...
                <name type="conference">
                    ...
                    <namePart>
                        <xsl:value-of
                            select="mods:stripPunctuation(mods:specialSubfieldSelect(.,'aqdc','t','gn',''))"
                        />
                    </namePart>
                    <xsl:apply-templates
                        select="marc:subfield[@code='e'] | marc:subfield[@code='4']" mode="role"/>
    ...
    <xsl:template match="marc:subfield[@code='g']" mode="relatedItem">
        <xsl:if
            test="parent::marc:datafield[@tag='760' or @tag='762' or @tag='765'     or @tag='767' or @tag='770' or @tag='772' or @tag='774' or @tag='775'    or @tag='776' or @tag='777' or @tag='780' or @tag='780' or @tag='785' or    @tag='786' or @tag='787']">
            <partNumber>
                <xsl:value-of select="normalize-space(mods:stripPunctuation(.))"/>
            </partNumber>
        </xsl:if>
    ...
    <xsl:template
        ...
        <identifier>
            ...
            <xsl:choose>
                <xsl:when test="@ind1='0'">
                    <xsl:value-of select="mods:subfieldSelect(.,'ba')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="mods:subfieldSelect(.,'ab')"/>
                </xsl:otherwise>
...
<!-- IID identifier for FSU records -->
    <xsl:template match="marc:datafield[@tag='050'] | marc:datafield[@tag='090']">
         <xsl:choose>
            ...
            <xsl:when test="starts-with(.,'urn:hdl') or starts-with(.,'hdl')">
                <identifier type="hdl">
                    ...
                    <xsl:if test="marc:subfield[@code='y' or @code='3' or @code='z']">
                        <xsl:value-of select="mods:subfieldSelect(parent::*,'y3z')"/>
                    </xsl:if>
    ...
    <xsl:template name="nameTitleGroup">
        <xsl:choose>
            ...
            <xsl:when
                ...
                <xsl:choose>
                    <xsl:when
                        test="../marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'100')] or        ../marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'110')] or       ../marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'111')]">
            ...
            <xsl:when
                test="(self::marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'100')]     or self::marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'110')]     or self::marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'111')])">
                <xsl:choose>
                    <xsl:when
            ...
            <xsl:when
                test="self::marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'100')]      | self::marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'110')]      | self::marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'111')] "/>
            <xsl:when
